...
-- Практическое задание к видеоуроку по теме "Сложные запросы"
...
COMMENT = 'Список полетов';

DROP TABLE IF EXISTS cities;

CREATE TABLE cities (
	label VARCHAR(255) PRIMARY KEY COMMENT 'Английское название города',
	name VARCHAR(255) COMMENT 'Русское название города')
COMMENT = 'Сопоставление названий городов';
...
SELECT
	...
	(
		...
		FROM
			cities
		WHERE
			label = f.`from`) AS 'Город отправления', 
	(
		SELECT
			name
		FROM
